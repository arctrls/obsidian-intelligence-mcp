...
class SemanticSearchServiceTest {
    ...
    private val keywordSearchService = mockk<KeywordSearchService>()
    ...
    @BeforeEach
    fun setUp() {
        service = SemanticSearchService(vectorStore, searchProperties, keywordSearchService)
    }
    ...
    @Nested
    inner class HybridSearch {

        @Test
        fun `combines dense and keyword scores for hybrid search`() {
            val doc1 = createAiDocument(
                filePath = "/vault/kotlin.md",
                title = "Kotlin Guide",
                tags = "kotlin",
                content = "kotlin programming language guide",
                score = 0.9
            )
            val doc2 = createAiDocument(
                filePath = "/vault/java.md",
                title = "Java Guide",
                tags = "java",
            ...
            every { vectorStore.similaritySearch(any<SearchRequest>()) } returns listOf(doc1, doc2)
            every { keywordSearchService.calculateScore("Kotlin Guide", listOf("kotlin"), any(), any()) } returns 5.0
            every { keywordSearchService.calculateScore("Java Guide", listOf("java"), any(), any()) } returns 0.0

            val results = service.search(
                SearchQuery(text = "kotlin", searchType = SearchType.HYBRID)
            )

            assertThat(results).hasSize(2)
            assertThat(results[0].filePath).isEqualTo("/vault/kotlin.md")
        }

        @Test
        fun `hybrid search returns results sorted by combined score`() {
            val doc1 = createAiDocument(
                filePath = "/vault/a.md",
                title = "Low Dense High Keyword",
                score = 0.5,
                content = "kotlin kotlin kotlin"
            )
            val doc2 = createAiDocument(
                filePath = "/vault/b.md",
                title = "High Dense Low Keyword",
                score = 0.95,
                content = "unrelated"
            )

            every { vectorStore.similaritySearch(any<SearchRequest>()) } returns listOf(doc1, doc2)
            every { keywordSearchService.calculateScore("Low Dense High Keyword", any(), any(), any()) } returns 10.0
            every { keywordSearchService.calculateScore("High Dense Low Keyword", any(), any(), any()) } returns 0.0
    ...
    @Nested
    inner class KeywordSearch {

        @Test
        fun `keyword search ranks by keyword score`() {
            val doc1 = createAiDocument(
                filePath = "/vault/a.md",
                title = "Kotlin Guide",
                tags = "kotlin",
            ...
            every { vectorStore.similaritySearch(any<SearchRequest>()) } returns listOf(doc1, doc2)
            every { keywordSearchService.calculateScore("Kotlin Guide", listOf("kotlin"), any(), any()) } returns 8.0
            every { keywordSearchService.calculateScore("Java Guide", listOf("java"), any(), any()) } returns 0.0

            val results = service.search(
                SearchQuery(text = "kotlin", searchType = SearchType.KEYWORD)
            )

            assertThat(results).hasSize(1)
            assertThat(results[0].filePath).isEqualTo("/vault/a.md")
        }
    }
